...
# 전처리(stem = True, norm = True)
def preprocess(text):
    # Okt 특정 품사 추출(stem = True, norm = True)
    okt = Okt()
    pattern = 'Noun|Verb|Adjective|Adverb'
    text_list = pos_filter(pattern, okt.pos(text, stem = True, norm = True))
